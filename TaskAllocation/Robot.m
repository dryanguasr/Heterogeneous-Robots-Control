classdef Robot
    properties
        ...
        Nvf = 0;             % Número de vecinos hasta la frontera
        % Evacuación de Víctimas
        VictimID = [];       % Identificador de víctima encontrada
        VictimTar = 0;       % ID de la víctima objetivo actual
        VictimPos = [];      % Última posición conocida de las víctimas halladas
        VictimStatus = [];   % Estado de las víctimas (0-ND, 1-Mov, 2-inMov, 3-Falsa)
        HomePos = [];        % Posición de Home para evacuación y retorno
        % Funciónes de potencial de atracción y repulsión
        PotNet = [0;0];
        % Parámetros del Controlador
        Kp = 10;             % Constante proporcional para seguimiento de posición
        Kar = 10;            % Constante proporcional para atracción/repulsión
        ...
        CurrentTask = 1;     % Posición actual en la lista de tareas
        Tc = [1 1 1 1];      % Capacidad para realizar tareas
        % Conectivdad
    ...
    methods
    % Constructor
        function r = Robot(ID, MW, MH, Mres, pos, V0, Sr, Cr, Kp, Tc)
    ...
    %% Cálculo de la distancia 
        ...
        % distancia por inundación euclidea (muy costoso)
        function r = euclideanFlood(r)
            r.Flood = r.MW/r.Mres * r.MH/r.Mres * ones(size(r.Map));
            for i=1:1:size(r.Map,1)
                for j=1:1:size(r.Map,2)
                    if(r.Map(i,j) == 2)
            ...
            while(sum(sum(abs(r.Flood-dPile))) ~=0 && cicle < MaxCicles)
                ...
                cicle = cicle + 1;
                %up-down, left-right cicle
                for i=2:1:size(r.Map,1)-1
                    for j = 2:1:size(r.Map,2)-1
                        for x = -1:1:1
                            for y = -1:1:1
                                if(r.Flood(i,j) ~= -1)
                                    canD = r.Flood(i,j) + sqrt(x^2+y^2);
                                    if(r.Flood(i+x,y+j) > canD)
                                        r.Flood(i+x,y+j) = canD;
                                    end
                                end
                            end
                        end
                ...
                %down-up, right-left cicle
                for i=size(r.Map,1)-1:-1:2
                    for j = size(r.Map,2)-1:-1:2
                        for x = -1:1:1
                            for y = -1:1:1
                                if(r.Flood(i,j) ~= -1)
                                    canD = r.Flood(i,j) + sqrt(x^2+y^2);
    ...
    %% funciones utilitarias
        function pV = goTo(r,p0)
            p = round(p0./r.Mres);
            counter = 0;
            while(r.LoS(p(1),p(2)) ~= 1 && counter < 100) % p is not visible
                counter = counter + 1;
                for i = -1:1:1
                    for j = -1:1:1
                        if(r.Flood(p(1)+i,p(2)+j) < r.Flood(p(1),p(2)) && r.Flood(p(1)+i,p(2)+j) ~= -1)
                            p = [p(1) + i; p(2)+j];
                        end
                    end
                end
    ...
    %% Funciones de mantenimiento de la comunicación
        ...
        function r = conectToVictim(r,victim)
            Drv = norm(r.Pos-victim.Pos);
            SrMin = min(r.Sr,victim.Sr);
            PotDir = (r.Pos-victim.Pos)/Drv;
            if(Drv<SrMin)
                % determines the type of the victim if possible
                if(r.Tc(3) == 1 && r.VictimStatus(victim.ID) ~= 4)
                    r.VictimStatus(victim.ID) = victim.Type;
                elseif(r.Tc(3) == 0 && r.VictimStatus(victim.ID) ~= 4)
                    r.VictimStatus(victim.ID) = 3;
                end
                if(r.VictimStatus(victim.ID) == 2)
                    % if the victim type is false there will not be attention
                    r.VictimStatus(victim.ID) = 4;
                end
                % colltion avoiding potential
                PotNorm = -1/Drv^2;
                % tracking potential if needed for delivery or evacuation
                if(victim.Type == 0 && r.VictimTar == victim.ID)
                    PotNorm = Drv-victim.Cr + r.V0/(SrMin^2-Drv^2)-1/Drv^3;
                end
                r.PotNet = r.PotNet + PotNorm*PotDir;
            end
        end
